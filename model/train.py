    ...
    for iter in range(iter_old + 1, total_iter):
        ...
        if iter % 1000 == 0:
            ...
            for _ in range(validation_steps):
                model_encoder.train(False)
                model_decoder.train(False)

                # load validation dataset
                imgs, textures, masks = data_set.feed(batch_size, mode='validation')
                ...
                encoder_img = nn_F.normalize(encoder_img)
                encoder_texture = nn_F.normalize(encoder_texture)
                # 把 ref texture的 encoding输出做尺度变换，（放大到128*128*64和32*32*64）再分别做corr，然后结果concate出来进decoding network
                texture_upsample = nn.Upsample(size=(128, 128), mode='bilinear')(encoder_texture).to(device)
                texture_downsample = nn.AdaptiveAvgPool2d((32, 32))(encoder_texture).to(device)

                filt = filt_adp(encoder_texture).to(device) # 将ref tex embedding降采样到5*5
                ...
                # iou
            real_loss = validation_loss / validation_steps
            print("Validation loss at iter ", iter, " is:", real_loss)
            # print(iou)


        else: # train
            model_encoder.train(True)
            model_decoder.train(True)
            ...
            model_decoder.zero_grad()
            model_encoder.zero_grad()
            # only do back-prop and update in train phase
            loss.backward()
            optimizer.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--segmentation_regions', type=int, default=5, help='number of segmentation_regions')
